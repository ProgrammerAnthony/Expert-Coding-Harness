...
**与「同会话逐步执行」的区别：** 本技能强调**每任务新子代理 + 双审查**；若用户选择在当前会话手写每一步、不用 Task 派子代理，可改按 `references/inline-execution-hint.md` 配合 **TDD 开发大师** 与 **代码审查专家**，不再套用下文的 Task 模板。

---

## 适用条件

- 已有实施计划（推荐由 **实施计划编写** 产出），任务粒度小、相对独立。
...
3. **对每个任务**循环：
   ...
   - 规格 ✅ 后派发 **代码质量审查**（`references/code-quality-reviewer-prompt.md`，对齐 **代码审查专家** 的严重度与证据要求）
   - 两道审查均未决问题后，勾选任务完成
4. **全部任务完成后**：建议对整体变更再做一次 **代码审查专家** 级别的汇总审查（可再派子代理），并请用户确认合并/PR；若项目后续提供「分支收尾」类技能可再衔接。
...
| 技能 | 作用 |
|------|------|
| **实施计划编写** | 产生本技能消费的勾选式计划 |
| **TDD 开发大师** | 实现子代理编写与运行测试时应遵循 RED-GREEN-REFACTOR；计划中已拆步骤时以计划为准 |
| **代码审查专家** | 代码质量审查的输出结构（P0–P3、证据）与此技能一致；全量收尾可再显式调用该技能 |
| **调试专家** | 实现或测试失败时，实现子代理或主会话可切换排查 |
...
更多叙事化示例见 superpowers 上游项目中的同名技能；本仓库为中文流程与 **代码审查专家** 对齐版。
