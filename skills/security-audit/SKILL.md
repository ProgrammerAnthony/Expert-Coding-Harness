---
name: security-audit
description: "Use when 用户需要检查代码安全性、发现潜在漏洞、上线前安全评估、怀疑代码存在注入/认证/授权等安全问题时。触发场景：安全审计、security audit、代码安全、安全扫描、漏洞检测、安全检查、渗透测试、安全评估、有没有安全漏洞、SQL注入、XSS、上线前安全review。"
---
...
遇到以下想法，立刻停下——无证据的安全结论比没有结论更危险：

| 借口 | 现实 |
|------|------|
| "这个框架通常有 SQL 注入，直接标记" | "通常有"不是证据。必须找到具体的代码调用路径才能标记漏洞。 |
| "看起来没有安全问题，可以跳过某个维度" | 安全问题藏在"看起来没问题"的地方。必须按 10 维度逐项扫描。 |
| "这个漏洞很明显，不需要完整调用链" | 没有完整调用链 = 标记为"可疑，需人工验证"，不得列为确认漏洞。 |
| "Quick 模式足够了，节省时间" | Quick 模式覆盖面有限。上线前正式安全评估必须用 Standard 或 Deep。 |
| "代码量太大，只扫重点部分" | 只扫重点意味着可能遗漏高危漏洞。必须完整覆盖攻击面清单。 |
| "不确定这个版本是否受 CVE 影响，就不提了" | 不确定要如实说明，提供排查方向。沉默不是解决不确定性的方式。 |
